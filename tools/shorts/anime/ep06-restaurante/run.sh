...
# --- SHOT 8 키프레임 (0.5 cr) ---
higgsfield generate create gpt_image_2 --aspect_ratio 9:16 --image charsheet.png --wait --prompt "$(cat prompts/08.txt)"
...
higgsfield generate create kling3_0_turbo \
  ...
  --prompt "Slow push in toward the family. The waiter raises his hand once. The door swings shut behind them. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 2 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/02.png \
  --prompt "Static shot. The waiter passes a menu across the table. HIJO settles onto his knees. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 3 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/03.png \
  --prompt "Static shot. HIJA lowers the menu slightly. The waiter taps the menu once. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 4 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/04.png \
  --prompt "Static shot. HIJO's raised arm wobbles. PADRE lifts one finger without moving his head. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 5 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/05.png \
  --prompt "Slow push in toward the pan. Steam curls rise. All four lean in together once. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 6 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/06.png \
  --prompt "Static shot. HIJO chews twice. MADRE's smile widens slightly. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 7 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/07.png \
  --prompt "Static shot. PADRE raises one hand. The waiter nods once and turns. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait

# --- SHOT 8 영상 (7.5 cr) ---
higgsfield generate create kling3_0_turbo \
  --start-image frames/08.png \
  --prompt "Static shot. HIJO leans further forward. PADRE does not move at all. Background static." \
  --duration 5 --resolution 1080p --aspect_ratio 9:16 --wait
